...
class _BaseStateState extends State<BaseState> with TickerProviderStateMixin {
        ...
        children: <Widget>[
          Expanded(
            ...
            child: Text(
              property,
              style:
                  TextStyle(fontSize: _getFontSize(15), color: Colors.black54),
            ),
          ),
          Text(
            value.toString(),
  ...
  Widget _shape() {
    ...
    return _propertyRow('Corpo', state.pokemonSpecies.shape.name);
  }

  Widget _seenAt() {
    final state = Provider.of<PokemonState>(context);
    if (state.pokemonSpecies == null ||
        state.pokemonSpecies.palParkEncounters == null ||
        state.pokemonSpecies.palParkEncounters.length == 0) {
      return Container();
    }

    return _propertyRow(
        'Onde vive', state.pokemonSpecies.palParkEncounters.first.area.name);
  }
  ...
  Widget _pokemonColor() {
    ...
    return _propertyRow('Cor principal', state.pokemonSpecies.color.name);
  }
  ...
  Widget _captureRate() {
    ...
    return _propertyRow(
        'Índice de captura', state.pokemonSpecies.captureRate.toString());
  }
        ...
        children: <Widget>[
          ...
          Expanded(
              ...
              child: Text(
                value,
                style: TextStyle(
                    fontSize: _getFontSize(14), color: Colors.black87),
              ))
        ],
      ),
    );
